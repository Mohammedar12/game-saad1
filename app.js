...
function handleCharacterCollisions() {
    ...
    if ( colisionBlock && !colisionHole ) {
        ...
        soundCount++
        if ( soundCount > 35 ) {
            (new Audio('/sounds/2WgGYe1Xsxk_bestaudio_mp3_0_1.1_637699382829251064.mp3')).play()
            soundCount = 0
        }
...
function gameOver() {
    (new Audio('/sounds/رياكشن القم ياهطف.mp3')).play()
    gameStopped = true
    showGameoverscreen()
    stopBlockAnimation()
    stopGravity()
...
function changeScoreUi() {
    score.innerText = `Your Age ${ scoreTotal.toString() }`
